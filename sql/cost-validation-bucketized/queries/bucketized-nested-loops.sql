...
join t1000000m m
  on m.c1 = t.c1
 and m.c3 = t.c3
where t.bucketid in (0,1,2)
  and (yb_hash_code(m.c1, m.c3) % 3) in (0,1,2)
order by m.c1, m.c3;
...
join t1000000m m
  on m.c1 = w.c1
 and m.c2 = w.c2
where (yb_hash_code(w.c1, w.c2) % 3) in (0,1,2)
order by w.c1, w.c2;
...
join t100000w w
  on w.c1 = m.c1
where (yb_hash_code(m.c1) % 3) in (0,1,2)
  and (
        m.c3 in (5,10,15)
...
lateral (
    select *
    from t1000
    where (yb_hash_code(c2, c4) % 3) = buckets.b
      and c3 < 50
      and c5 > 10
    order by c2, c4
    limit 60
...
lateral (
  ...
  from t1000000m
  where c1 = r.c1
    and (yb_hash_code(c1,c3)%3) in (0,1,2)
  order by c1, c3
  limit 10
) x
order by r.c1, r.c2;
...
with s as (
  select c1, v
  from t10000
  where (yb_hash_code(c1,v)%3) in (0,1,2)
)
...
lateral (
  ...
  where c1 = s.c1
    and (yb_hash_code(c1,c3)%3) in (0,1,2)
    and c3 not in (
        select c3 from t1000 where bucketid in (0,1,2)
    )
    and length(s.v) > 0
) x
order by s.c1;
...
lateral (
  ...
  full join t10000 b
    on a.c1 = b.c1
  where a.c1 = seed.c1
    and (yb_hash_code(a.c1,a.c2)%3) in (0,1,2)
) t
order by t.c2, t.c3;

SELECT DISTINCT ON (c1,c2) c1, c2, c3, c4
FROM t1000000m
...
AND (yb_hash_code(t.c1,t.c2)%3) IN (0,1,2)
ORDER BY c1, c2;
