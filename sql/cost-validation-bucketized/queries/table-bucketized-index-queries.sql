...
SELECT c1, c2, c3 FROM table_bucketized WHERE c1 BETWEEN 100 AND 50000 and c2=10 ORDER BY c1 DESC, c2 DESC LIMIT 500;
...
SELECT c2, c3, rank() OVER (PARTITION BY c2 ORDER BY c3) AS rnk, count(*) OVER (PARTITION BY c2) AS cnt FROM table_bucketized ORDER BY c2, c3;
SELECT c2, length(v) AS vlen, row_number() OVER (PARTITION BY c2 ORDER BY length(v)) AS rn FROM table_bucketized ORDER BY c2, v;
SELECT c2, c4, avg(c5) OVER (ORDER BY c2 ROWS BETWEEN 50 PRECEDING AND CURRENT ROW) AS mov_avg FROM table_bucketized WHERE c2 > c4 ORDER BY c2, c4;
SELECT c2, c3, count(*) OVER (PARTITION BY c3) AS cnt, avg(c5) OVER (PARTITION BY c2) AS avg_c5 FROM table_bucketized WHERE c2 BETWEEN 1000005 AND 1005000;
SELECT c2, c5, sum(c6) OVER (ORDER BY c5 ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS run_sum FROM table_bucketized WHERE c5 BETWEEN 1000 AND 90000 ORDER BY c2, c5;
