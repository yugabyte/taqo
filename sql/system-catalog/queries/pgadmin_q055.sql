SELECT rel.oid, rel.relname AS name, rel.reltablespace AS spcoid,rel.relacl AS relacl_str,
  (CASE WHEN length(spc.spcname::text) > 0 OR rel.relkind = 'p' THEN spc.spcname ELSE
    ...
    WHERE dtb.oid = (select oid from pg_database where datname = 'taqo_system_catalog'))
  END) as spcname,
  ...
  (select nspname FROM pg_catalog.pg_namespace WHERE oid = 'public'::regnamespace ) as schema,
  ...
  (SELECT count(*)
	...
	CASE WHEN typ.typname IS NOT NULL THEN (select pg_catalog.quote_ident(nspname) FROM pg_catalog.pg_namespace WHERE oid = 'public'::regnamespace )||'.'||pg_catalog.quote_ident(typ.typname) ELSE typ.typname END AS typname,
...
WHERE rel.relkind IN ('r','s','t','p') AND rel.relnamespace = 'public'::regnamespace
